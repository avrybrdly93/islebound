...
### BL-083 — Nothing asserts that the type-aware lint rules stay on over `tools/` — **DONE 2026-09-23**
...
- **Acceptance criteria:**
  - [x] A deliberate type-aware violation in a `tools/` file that is inside `tools/tsconfig.json` is asserted to report, by something `pnpm test` or `pnpm lint:rules` runs — **`pnpm lint:rules`**, a fifth row in `tools/check-lint-rules.ts`'s `EXPECTATIONS` over the new `tools/type-aware-fixtures/unnecessary-condition.ts`. `tools/tsconfig.json` needed no edit: its `include` is already `"**/*.ts"` and only `lint-fixtures` is excluded, so the new directory is type-checked by construction
  - [x] The assertion fails if `tools/**/*.ts` is returned to the `disableTypeChecked` block — verified by doing it, not by reading the config — **done, and the four existing rows stayed green while the new one reported 0**, so the failure is attributable rather than a whole-file collapse. The same run confirms what makes the regression worth guarding: under it `pnpm lint` exits 0, `pnpm typecheck` exits 0 and `pnpm test` is **372/372**. Reverted, and 5/5 restored
  - [x] It does not fire on the syntactic rules, which were never off there and would mask the regression — **measured, not merely designed for**. The row is keyed on `ruleId` (`@typescript-eslint/no-unnecessary-condition`), which a syntactic rule cannot satisfy; and under the restored block ESLint reports **zero messages of any kind** on the fixture, so there is nothing to mask it with
- **Outcome:** decision **0039**. The fixture directory is the inverse of `tools/lint-fixtures/` on exactly one axis — in ESLint's `ignores`, and in **neither** the `disableTypeChecked` block nor the tsconfig's `exclude` — and the top of `eslint.config.js` now says which directory a new fixture belongs in and how to tell. **It also rehabilitates the control 0038 recorded as a miss:** that session's `no-unnecessary-condition` probe reported the stylistic `no-inferrable-types` instead and proved nothing, and the difference is the *shape of the violation*, not the rule — an annotated initializer attracts a stylistic rule first, a comparison against `undefined` on a non-nullable type attracts only the type-aware one. Suite unchanged at **372 pass / 0 fail across 90 suites** (this task adds no `pnpm test` case; its assertion is `lint:rules`'s fifth, 4 → 5 fixtures caught). `lint`, `typecheck`, `test`, `lint:rules` and `lint:docs` all clean. Filed **BL-084**
...
### BL-084 — Nothing automatic runs `pnpm lint:rules`, so all five of its assertions depend on a session typing it
- **Phase:** 0 · **Size:** S · **Depends on:** BL-019 · **Docs to read:** 29, 30
- **Description:** BL-083 put the type-aware wiring behind `pnpm lint:rules`, which is the fifth assertion in that script — and **the verify block does not run it.** `CLAUDE.md`, `AI_DEVELOPMENT_WORKFLOW.md` §6 and `35_AI_AGENT_RULES.md` §8.1 all say `pnpm lint && pnpm typecheck && pnpm test`; `lint:rules` and `lint:docs` appear only in the prose beneath the block, which decision **0035**'s own context already identified as the weak position (`pnpm format:check` sat there for months while four files went unformatted). So five gates — the four custom `no-restricted-syntax` rules from BL-002 and now the type-aware family — are only as good as a session remembering a command the block does not name. This is not a defect BL-083 introduced; it is the standing shape of `lint:rules` since BL-002, made worth filing by a fifth assertion moving in.
- **Acceptance criteria:**
  - [ ] `pnpm lint:rules` is reached by something a session or CI runs without having to name it
  - [ ] Whatever carries it is **not** `pnpm lint` — see the notes; that placement is rejected on a measured finding, not on taste
  - [ ] The carrier is itself asserted, so this does not become the seventh instance of the defect it closes
- **Notes:** Filed 2026-09-23 by BL-083 (`35` §3 — discovered work, not done inline). **Two of the obvious answers are already rejected with reasons, so start from the third.** (a) *Chain it into `pnpm lint`*, which is exactly how 0033 and 0035 made `format:check` unskippable. **Rejected on BL-079's finding 1: a guard cannot live inside the thing it guards.** `lint:rules` exists to catch a broken `eslint.config.js`, and the edit it guards against would take the guard down with it. (b) *Add a fourth line to the verify block.* Rejected by decision **0035** already — a line in a document is precisely what gets forgotten, which is why BL-077 happened at all. (c) The live candidate is therefore **`pnpm test`**: it is in the block, it already runs `tools/**/*.test.ts`, and `tools/check-lint-script.test.ts` is the existing precedent for a test that asserts about tooling. Moving the `EXPECTATIONS` table into a `.test.ts` would do it, and `check-lint-rules.ts`'s own header already names that as its eventual destination under BL-015. **Consider waiting for BL-019 instead**: CI can run every gate by name and makes this moot, which is why this item depends on it rather than racing it. If BL-019 lands first, close this by checking that its workflow names `lint:rules`, and assert that too.

### BL-078 — The allocation boundary cannot see an operation that allocates rarely — **DONE 2026-09-26**
...
- **Acceptance criteria:**
  - [x] A sparse allocator — one object per 1000 calls — is caught on every run of 20, not most — **20 of 20 full-suite runs green**, each of which asserts it twice (`allocationControls.test.ts` and `allocation.test.ts`'s `before`). The margin, not just the sign: the sparse control reads **58.9–79.1 sampling intervals** against an allowance of 4, a minimum of **14.7×** where the old interval gave **1.6×**
  - [x] Whatever achieves that does **not** reintroduce BL-074: a single stray sample must still not fail an operation whose true reading is zero — **`MAX_STRAY_SAMPLES` is untouched at 4**, which is how this is met by construction rather than by re-measurement, and a test asserts it stayed 4 so a later change cannot meet the first criterion by widening the boundary. An allocation-free operation read **0 in 20 of 20 runs** at the new interval
  - [x] The per-call control still passes its gap assertion, so the instrument is still known to resolve a real allocator — **87.3×** the allowance against a required 8×, up from 19.6×
- **Notes:** Filed 2026-09-12 by BL-074, which measured it and deliberately did not chase it (`35` §3 — the item was a boundary, not an instrument redesign). **Raising `MAX_STRAY_SAMPLES` moves the boundary the wrong way and lowering it re-creates BL-074**, so this is not a constant to retune; it needs a different instrument. The two obvious axes are a longer measured window and a finer `samplingInterval`, and **both have measured failure modes already recorded in `allocationHarness.ts`**: at interval 16 an allocation-free operation read 904 bytes where 64–8192 all read exactly 0, and a warm-up of 200000 made the *control* read 0 in one pass of three. Read that options table before picking either. Note also that the gap assertion caps the allowance from above: on the reference container the control backs at most **6** intervals, measured, so there is no room to widen the boundary toward the sparse case even if that seemed attractive. **CLOSED 2026-09-26 by moving `DEFAULT_SAMPLING_INTERVAL` from 1024 to 256, and by nothing else.** The mechanism is the part worth keeping, because it is not "finer sampling is more accurate": a stray is **one sample**, so the four-interval allowance is four *samples* at any interval, while the reading a sparse allocator produces in **bytes** is very nearly interval-independent — about 18 kB at 1024, 256 and 64 alike, because the sampling profiler's attribution saturates far below true allocated volume. So a finer interval multiplies the sparse allocator's *sample* count and leaves the stray tolerance exactly where BL-074 put it. That is why this was neither "raise `MAX_STRAY_SAMPLES`" nor "lower it". **The other axis was measured and rejected**: 1 000 000 iterations at interval 1024 reaches a 19.9× margin but costs **257 ms** a measurement against 256's 178 ms, and at that setting the control and the sparse case *overlap* (73–102 against 80–94 intervals), so the instrument has saturated and the reading has stopped being about volume at all. The item's warning about a finer interval is about **16**, where an allocation-free operation read 904 bytes; the harness's own options table records 64, 256, 1024 and 8192 all reading exactly **0**, so 256 is inside the measured-safe band. **New file** `core/math/allocationControls.ts` (`createAllocator`, `SPARSE_ALLOCATION_PERIOD`, `MIN_SPARSE_TO_ALLOWANCE_RATIO`, `assertInstrumentResolvesSparseAllocator`) plus `allocationControls.test.ts` — a new file because `allocationHarness.ts` was at 496 of its 500 lines and because the instrument and the known signals used to judge it are different things. **Both controls come from one factory differing only in `period`**, so "the sparse one reads lower" cannot be a property of two different fixtures. **Suite 372/90 → 381/91, and 3.96 s → 5.34 s** — the cost of the finer interval, named rather than hidden. **Eight controls, each with the tree restored**: coarsening the interval back to 1024 fails the sparse assertion with its own diagnosis (12 672/14 976 bytes against a required 16 384); 65536 fails 3; 16 fails 2 (the `vec3` suites, the documented noise direction); raising `MAX_STRAY_SAMPLES` to 64 fails 4; a sparse control that allocates every call fails 2; one that never allocates fails 5; dropping `keepAlive` fails 5; sharing one counter across allocators is rejected by `pnpm typecheck`. **One control produced no failure and it is recorded as a gap, not as a pass**: a warm-up of 200 000 — which the harness documents as making the *control* read 0 in one pass of three — left 26/26 green, because a one-in-three failure cannot be demonstrated either way by one mutant run. Filed as **BL-086**. **Still open one decade out:** 1 in 10 000 reads 4.9–9.8 intervals and is therefore caught on most runs and not all — the same shape this item closed. Filed as **BL-085** rather than quietly attempted, since the next factor of four costs 384 ms a measurement.

### BL-085 — The allocation boundary still cannot see one allocation per 10 000 calls
- **Phase:** 0 · **Size:** M · **Depends on:** BL-078 · **Docs to read:** 29
- **Description:** BL-078 moved the sampling interval to 256 and brought a 1-in-1000 allocator from 1.6× the stray allowance to 14.7×, which is criterion-met and caught on every run of 20. One decade further out is unchanged in shape: **one allocation per 10 000 calls reads 4.9–9.8 sampling intervals** against an allowance of 4, so it is caught on most runs and not all — which is exactly the state BL-078 existed to end, one order of magnitude out. An operation allocating that rarely still violates `CLAUDE.md`'s no-allocation-in-per-frame-paths rule.
- **Acceptance criteria:**
  - [ ] A 1-in-10 000 allocator is caught on every run of 20, not most
  - [ ] `MAX_STRAY_SAMPLES` is still not the knob, and the per-call and 1-in-1000 controls both still pass
  - [ ] Whatever it costs is measured and written down, because the obvious route is the expensive one
- **Notes:** Filed 2026-09-26 by BL-078. **Read `allocationControls.ts`'s header before choosing**, and note the arithmetic that makes this harder than BL-078 rather than the same again: the interval axis buys a factor of four for a factor of 2.4 in time (1024→256 was 72 ms→178 ms a measurement), and the next step, interval 64, costs **384 ms** — which on ~32 measurements is about 12 s on a suite that is 5.3 s today. So this is a genuine trade rather than a free win, and the answer may be "measure a sparse operation on demand rather than on every run". **Do not go to interval 16**: an allocation-free operation reads 904 bytes there, measured, where 64–8192 all read exactly 0. The iteration axis is worse than it looks — at 1 000 000 iterations and interval 1024 the per-call control and the 1-in-1000 case overlap, so the instrument saturates before it discriminates. **Nothing is blocked on this**; no per-frame path in the repository allocates at all today.

### BL-086 — Nothing pins the measurement window's warm-up, and the failure it guards is intermittent
- **Phase:** 0 · **Size:** S · **Depends on:** — · **Docs to read:** 29
- **Description:** `measureAttributedAllocation`'s `warmup` defaults to 50 000, and the harness documents both directions of getting it wrong: at 5000 an allocation-free operation reads 3–10 kB in the first of three passes, and **at 200 000 the *control* read 0 in one pass of three**, which is the dangerous one — a warm-up long enough to leave the operation's allocation unsampled makes a real allocator look clean. Nothing asserts the default. BL-078 ran a control that set it to 200 000 and **the suite stayed 26/26 green**, so the guard that ought to catch it does not.
- **Acceptance criteria:**
  - [ ] A warm-up that blinds the instrument fails something, and what it fails is stated
