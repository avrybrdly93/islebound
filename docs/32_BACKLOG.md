...
### BL-069 — The 500-line limit is still unenforced, and three test files are over it — **DONE 2026-08-31**
...
- **Acceptance criteria:**
  - [x] Either `max-lines` is enabled at the hard limit and every file is under it, or `CLAUDE.md`'s limit is restated as a convention nobody enforces and the reason is written down — not left as a number that reads like a rule
  - [x] If enabled, the *soft* limit is decided too: a `warn` at 300 that nothing fails on is noise unless somebody reads warnings, and this repository's lint runs in CI where nobody does
  - [x] Whatever is chosen, whether test files are in scope is stated explicitly — they are the files that actually overrun, and `29` has no opinion on their length
...
**Done 2026-08-31**, see `34_DEVELOPMENT_LOG.md` and decision **0029**. `max-lines` is an **error at 500**, counting blank lines and comments, applied to **every** linted file including tests. The scoping is the decision: a sources-only rule would have enforced nothing at all — the largest source in the tree is `allocationHarness.ts` at **422**, while all three hard-limit violations are tests. The three (`World.test.ts` 547, `EventBus.test.ts` 533, `Rng.test.ts` 509) carry file-level suppressions naming their count and **BL-070**, rather than being split here; `35` §3 forbids that scope, and BL-068 recorded why a split is its own task with a before/after count. The **soft limit is deliberately not a rule of any severity** — lint runs in CI, which reads an exit code, so a `warn` nothing fails on is output nobody reads; 300 stays a convention and `CLAUDE.md` keeps saying so. **The rule is proved to fire**, through the repository's own `pnpm lint:rules` mechanism: `tools/lint-fixtures/over-max-lines.ts` is 501 lines of blanks and comments, and its expectation asserts on the line count in the message, so flipping `skipComments` to `true` takes the checker from 5/5 to a named failure — verified by doing it, not reasoned about. Suite unchanged at **342 pass / 0 fail**.

### BL-070 — Three test suites are over the 500-line hard limit and suppress the rule
- **Phase:** 0 · **Size:** S · **Depends on:** — · **Docs to read:** 06, 29
- **Description:** `World.test.ts` (**547**), `EventBus.test.ts` (**533**) and `Rng.test.ts` (**509**) each carry a file-level `eslint-disable max-lines` added by BL-069, which turned the rule on rather than fixing the files it caught. `Query.test.ts` is at **499** — one line under, and it crosses on the next case anybody adds.
- **Acceptance criteria:**
  - [ ] Each of the three is either under 500 lines or has a written reason it should not be, and its suppression is removed either way
  - [ ] The test count is recorded before and asserted after each split — a split that drops a `describe` block still runs green
  - [ ] `Query.test.ts` is addressed or explicitly left, with the choice stated: it is not in violation today and splitting it pre-emptively may be the wrong shape
- **Notes:** Filed 2026-08-31 by BL-069. **The failure mode is the whole task and it is silent**: BL-068 recorded that a split which drops or duplicates a `describe` block still passes, because the remaining cases pass and nothing reports the missing ones. Record the count first (342 pass / 88 suites as of 2026-08-31) and assert it after. **Two things worth deciding rather than assuming.** (1) Whether these suites should be split at all: `29_TESTING_STRATEGY.md` has no opinion on test-file length, and a suite that is long because it covers a lot may be better long than fragmented across files a reader has to reassemble — in which case the honest outcome is a documented exemption in `29`, not three splits. (2) If they are split, along what seam: BL-068's split followed the *subject* seams of the code under test (what a component is / what a store holds / which stores exist) rather than cutting at a line count, and that is the precedent to follow. **Not urgent** — the rule is on and every new file is subject to it, so this is paying down three known violations rather than stopping a leak.
